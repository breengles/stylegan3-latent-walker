...
from latent_walker import renderer
from latent_walker import pickle_widget
from latent_walker import latent_widget
from latent_walker import edit_widget
from latent_walker import keyframing_widget
from latent_walker import capture_widget

#----------------------------------------------------------------------------

class LatentWalker(imgui_window.ImguiWindow):
    def __init__(self, capture_dir=None):
        super().__init__(title='Latent Walker', window_width=3840, window_height=2160)
        ...
        self.keyframing_widget  = keyframing_widget.KeyframingWidget(self)
    ...
    def draw_frame(self):
        ...
        expanded, _visible = imgui_utils.collapsing_header('Feature editor', default=True)
        self.edit_widget(expanded)
        self.capture_widget(expanded)
        imgui.new_line()
        expanded, _visible = imgui_utils.collapsing_header('Keyframing', default=True)
        self.keyframing_widget(expanded)
...
@click.option('--capture-dir', help='Where to save/load tensors', metavar='PATH', default=None)
...
def main(
    ...
    """
    walker = LatentWalker(capture_dir=capture_dir)

    if browse_dir is not None:
        walker.pickle_widget.search_dirs = [browse_dir]

    # List pickles.
    if len(pkls) > 0:
        for pkl in pkls:
            walker.add_recent_pickle(pkl)
        walker.load_pickle(pkls[0])
    else:
        pretrained = [
        ...
        for url in pretrained:
            walker.add_recent_pickle(url)

    # Run.
    while not walker.should_close():
        walker.draw_frame()
    walker.close()
