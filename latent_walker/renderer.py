...
class Renderer:
    ...
    def _render_impl(self, res,
        ...
        img_scale_db    = 0,
        img_normalize   = False,
        display_mode    = 0, # 0:Feature editor, 1:Original, 2:Keyframing
        vs              = None,
        batch_edit      = dnnlib.EasyDict(layer_weights = []),
        current_frame   = dnnlib.EasyDict(layer_weights = [], w0_seeds=[[0, 1]]),
        ...
        if display_mode == 2: # keyframing
            w0_seeds = current_frame.w0_seeds
        ...
        # Apply PCA edits:
        wn = torch.clone(w)
        if vs is not None and display_mode != 1:
            if display_mode == 0:
                layer_weights = batch_edit.layer_weights
            else:
                layer_weights = current_frame.layer_weights
            device = torch.device('cuda')
            V = torch.from_numpy(vs.V).to(device)
            layers = 16
            components = 20
            mult = 3
            for layer_idx in range(layers):
                for comp_idx in range(components):
                    wn[:,layer_idx] += mult * V[layer_idx, comp_idx, :] * layer_weights[layer_idx, comp_idx]


        # Run synthesis network.
        synthesis_kwargs = dnnlib.EasyDict(noise_mode=noise_mode, force_fp32=force_fp32)
        torch.manual_seed(random_seed)
        out, layers = self.run_synthesis_net(G.synthesis, wn, capture_layer=layer_name, **synthesis_kwargs)
        ...
        if cache_key not in self._net_layers:
            if layer_name is not None:
                torch.manual_seed(random_seed)
                _out, layers = self.run_synthesis_net(G.synthesis, wn, **synthesis_kwargs)
